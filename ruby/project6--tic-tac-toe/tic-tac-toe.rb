	...
	def winning_move?
		...
		for x in 0..BOARD_SIZE-1
			for y in 0..BOARD_SIZE-1
				...
				p win << true if can_go_up_right?(x,y) && diagonal_row_up_same?(x,y) #evaluation stops if first expression returns false
			end
		end
...
firs_player_name = gets.chomp.to_s
puts "second player name?"
second_player_name = gets.chomp.to_s

b = Board.new
players = [Player.new(firs_player_name,"x"), Player.new(second_player_name, "o")].cycle

move = ""
b.show_board
puts "--------------------------------------------------------------------"
puts "Make a move by giving first the column number (horizontal), a space,"
puts "and then the row number (vertical). For example:"
puts "1 1"
puts "--------------------------------------------------------------------"
until (b.winner || b.board_full)
	current_player = players.next
	puts "+++++++++++++++++++++++++"
	puts "#{current_player.name}, please make your move."
	b.make_a_move(current_player)
	b.show_board
end